...
public:
    ...
    static bool HandleNpcBotWPSetFlagsZoneCommand(ChatHandler* handler, Optional<int32> flags)
    {
        ...
        WanderNode::DoForAllZoneWPs(zoneId, [handler = handler, flags = *flags](WanderNode const* wp) {
            ...
            if (flags < 0)
            {
                handler->PSendSysMessage("Removing WP {} '{}' flag {}", wpId, wp->GetName().c_str(), uint32(-flags));
                const_cast<WanderNode*>(wp)->RemoveFlags(BotWPFlags(-flags));
            }
            else
            {
                handler->PSendSysMessage("Adding WP {} '{}' flag {}", wpId, wp->GetName().c_str(), uint32(flags));
    ...
    static bool HandleNpcBotWPSetNameCommand(ChatHandler* handler, Optional<std::string> newname)
    {
        ...
        handler->PSendSysMessage("Changing WP {} '{}' name to '{}'", wpId, wp->GetName().c_str(), newname->c_str());
        wp->SetName(*newname);
    ...
    static bool HandleNpcBotWPMoveCommand(ChatHandler* handler, Optional<uint32> wpId)
    {
        ...
        handler->PSendSysMessage("WP {} '{}' was successfully moved.", wp->GetWPId(), wp->GetName().c_str());
        ...
        Optional<uint8> minlevel, Optional<uint8> maxlevel)
    {
        ...
        handler->PSendSysMessage("Created WP {} '{}' levels {}-{} flags {}", wpId, wpName.c_str(), uint32(minl), uint32(maxl), wpFlags);
    ...
    static bool HandleNpcBotWPDeleteCommand(ChatHandler* handler)
    {
        ...
        handler->PSendSysMessage("WP {} '{}' was successfully deleted.", wpId, wpName.c_str());
    ...
    static bool HandleNpcBotDumpLoadCommand(ChatHandler* handler, Optional<std::string> file_str, Optional<bool> forceKick)
    {
        ...
        switch (NPCBotsDump().Load(*file_str))
        {
            ...
            case BOT_DUMP_FAIL_FILE_NOT_EXIST:
                handler->PSendSysMessage("Can't open {} or the file doesn't exist!", file_str->c_str());
                handler->SetSentErrorMessage(true);
                return false;
            ...
            case BOT_DUMP_FAIL_DATA_OCCUPIED:
                handler->PSendSysMessage("Table data contained in {} overlaps with existing table entries!", file_str->c_str());
                handler->SetSentErrorMessage(true);
                return false;
            default:
    ...
    static bool HandleNpcBotDumpWriteCommand(ChatHandler* handler, Optional<std::string> file_str)
    {
        ...
        switch (NPCBotsDump().Write(*file_str))
        {
            ...
            case BOT_DUMP_FAIL_FILE_ALREADY_EXISTS:
                handler->PSendSysMessage("File {} already exists!", file_str->c_str());
                handler->SetSentErrorMessage(true);
                return false;
    ...
    static bool HandleNpcBotOrderCastCommand(ChatHandler* handler, Optional<std::string> bot_name, Optional<std::string> spell_name, Optional<std::string_view> target_token)
    {
        ...
        if (bot)
        {
            if (!bot->IsInWorld())
            {
                handler->PSendSysMessage("Bot {} is not found!", bot_name->c_str());
                return true;
            }
            if (!bot->IsAlive())
            {
                handler->PSendSysMessage("{} is dead!", bot->GetName().c_str());
                return true;
            }
            ...
            if (!base_spell)
            {
                handler->PSendSysMessage("{} doesn't have spell named '{}'!", bot->GetName().c_str(), spell_name->c_str());
                return true;
            }
            if (!canBotUseSpell(bot, base_spell))
            {
                handler->PSendSysMessage("{}'s {} is not ready yet!", bot->GetName().c_str(), sSpellMgr->GetSpellInfo(base_spell)->SpellName[handler->GetSessionDbcLocale()]);
                return true;
            }
        }
        else
        {
            ...
            {
                handler->PSendSysMessage("Unknown bot name or class {}!", class_name.c_str());
                return true;
            }
            ...
            if (!base_spell)
            {
                handler->PSendSysMessage("None of {} found bots have spell named '{}'!", cBots.size(), spell_name->c_str());
                return true;
            }
            ...
            if (!bot)
            {
                handler->PSendSysMessage("None of {} found bots can use {} yet!", cBots.size(), spell_name->c_str());
                return true;
            }
        }
        ...
        {
            if (DEBUG_BOT_ORDERS)
                handler->PSendSysMessage("Order given: {}: {} on {}", bot->GetName().c_str(),
                    sSpellMgr->GetSpellInfo(base_spell)->SpellName[handler->GetSessionDbcLocale()], target ? target->GetName().c_str() : "unknown");
        }
        else
        {
            if (DEBUG_BOT_ORDERS)
                handler->PSendSysMessage("Order failed: {}: {} on {}", bot->GetName().c_str(),
                    sSpellMgr->GetSpellInfo(base_spell)->SpellName[handler->GetSessionDbcLocale()], target ? target->GetName().c_str() : "unknown");
        }
            ...
            for (BotMap::const_iterator ci = bmap->begin(); ci != bmap->end(); ++ci)
            {
                ...
                if (bot && bot->GetVehicle())
                {
                    ...
                    if (doeject)
                    {
                        ...
                        //handler->PSendSysMessage("Removing {} from {}", bot->GetName().c_str(), bot->GetVehicle()->GetBase()->GetName().c_str());
                        bot->ExitVehicle();
                        //bot->BotStopMovement();
                    }
                }
    ...
    static bool HandleNpcBotSendToCommand(ChatHandler* handler, Optional<std::vector<std::string>> names)
    {
        ...
        static auto return_success = [](ChatHandler* chandler, Variant<std::string, uint32> name_or_count) -> bool {
            if (name_or_count.holds_alternative<uint32>())
                ...
                chandler->PSendSysMessage("Your next dest spell will send {} to position...", name_or_count.get<std::string>().c_str());
            return true;
        };
    ...
    static bool HandleNpcBotSendToLastCommand(ChatHandler* handler, Optional<std::vector<std::string>> names)
    {
        ...
        static auto return_success = [](ChatHandler* chandler, Variant<std::string, uint32> name_or_count) -> bool {
            if (name_or_count.holds_alternative<uint32>())
                ...
                chandler->PSendSysMessage("Moving {} to previous position...", name_or_count.get<std::string>().c_str());
            return true;
        };
    ...
    static bool HandleNpcBotSendToPointSetCommand(ChatHandler* handler, Optional<uint32> point_id, Optional<std::vector<std::string>> names)
    {
        ...
        static auto return_success = [&](ChatHandler* chandler, Variant<std::string, uint32> name_or_count) -> bool {
            if (name_or_count.holds_alternative<uint32>())
                ...
                chandler->PSendSysMessage("Marked send point {} for {}", *point_id, name_or_count.get<std::string>().c_str());
            return true;
        };
    ...
    static bool HandleNpcBotSendToPointCommand(ChatHandler* handler, Optional<uint32> point_id, Optional<std::vector<std::string>> names)
    {
        ...
        static auto return_success = [&](ChatHandler* chandler, Variant<std::string, uint32> name_or_count) -> bool {
            if (name_or_count.holds_alternative<uint32>())
                ...
                chandler->PSendSysMessage("Moving {} to point {}...", name_or_count.get<std::string>().c_str(), *point_id);
            return true;
        };
    ...
    static bool HandleNpcBotToggleFlagsCommand(ChatHandler* handler, Optional<uint32> flag)
    {
        ...
        handler->PSendSysMessage("Toggling flag {} on {}", setFlags, unit->GetName().c_str());
    ...
    static bool HandleNpcBotSetFactionCommand(ChatHandler* handler, Optional<std::string> factionStr)
    {
        ...
        handler->PSendSysMessage("{}'s faction set to {}", bot->GetName().c_str(), factionId);
        return true;
    }
    ...
    static bool HandleNpcBotSetOwnerCommand(ChatHandler* handler, Optional<std::string> charVal)
    {
        ...
        handler->PSendSysMessage("{}'s new owner is {} (guidlow: {})", bot->GetName().c_str(), characterName.c_str(), guidlow);
        return true;
    }
    ...
    static bool HandleNpcBotSetSpecCommand(ChatHandler* handler, Optional<uint8> spec)
    {
        ...
        handler->PSendSysMessage("{}'s new spec is {}", bot->GetName().c_str(), uint32(*spec));
        return true;
    }
    ...
    static bool HandleNpcBotLookupCommand(ChatHandler* handler, Optional<uint8> botclass, Optional <bool> unspawned, Optional<uint8> teamid)
    {
        ...
        for (BotList::const_iterator itr = botlist.begin(); itr != botlist.end(); ++itr)
        {
            ...
            handler->PSendSysMessage("{} - |cffffffff|Hcreature_entry:{}|h[{}]|h|r {}", itr->id, itr->id, itr->name.c_str(), raceName);
        }
    ...
    static bool HandleNpcBotDeleteCommand(ChatHandler* handler)
    {
        ...
        if (bot->GetBotAI()->IsWanderer())
        {
            ...
            handler->PSendSysMessage("Wandering bot {} '{}' successfully deleted", bot->GetEntry(), bot->GetName().c_str());
            return true;
        }
        ...
        {
            handler->PSendSysMessage("{} is unable to unequip some gear. Please remove equips before deleting bot!", bot->GetName().c_str());
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        handler->PSendSysMessage("Npcbot {} successfully deleted", bot->GetName().c_str());
        return true;
    }
    ...
    static bool HandleNpcBotDeleteByIdCommand(ChatHandler* handler, Optional<uint32> creature_id)
    {
        ...
        if (bot->GetBotAI()->HasRealEquipment())
        {
            ...
            {
                handler->PSendSysMessage("Cannot delete bot {} from console: has gear but no player to give it back to! Can only delete this bot in-game.", bot->GetName().c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }
            if (!bot->GetBotAI()->UnEquipAll(receiver))
    ...
    static bool HandleNpcBotCreateNewCommand(ChatHandler* handler, Optional<std::string> name, Optional<uint8> bclass, Optional<uint8> race, Optional<uint8> gender, Optional<uint8> skin, Optional<uint8> face, Optional<uint8> hairstyle, Optional<uint8> haircolor, Optional<uint8> features, Optional<uint8> soundset)
    {
        ...
        handler->PSendSysMessage("New NPCBot {} (class {}) is created with entry {} and will be available for spawning after server restart.", namestr.c_str(), uint32(*bclass), newentry);
        return true;
    }
    ...
    static bool HandleNpcBotGearScoreCommand(ChatHandler* handler, Optional<std::string_view> class_name)
    {
        ...
        if (class_name)
        {
            ...
            if (bot_class == BOT_CLASS_NONE)
            {
                handler->PSendSysMessage("Unknown bot name or class {}!", cname.c_str());
                return true;
            }
        ...
        for (Creature const* bot : bots)
        {
            ...
            handler->PSendSysMessage("{}'s GearScore total: {}, average: {}", bot->GetName().c_str(), uint32(scores.first), uint32(scores.second));
        }
    ...
    static bool HandleNpcBotInfoCommand(ChatHandler* handler, Optional<Variant<uint32, std::string>> player_lg_name)
    {
        ...
        if (master_guid.IsEmpty())
        {
            if (!master_name.empty())
            {
                handler->PSendSysMessage("Player '{}' is not found!", master_name.c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }
        ...
        {
            handler->PSendSysMessage("{} ({}) has no NpcBots!", master_name.c_str(), master_guid.GetCounter());
            handler->SetSentErrorMessage(true);
            return false;
        }
        ...
        handler->PSendSysMessage("Listing NpcBots for {}, guid {}{}:", master_name.c_str(), master_guid.GetCounter(), !master ? " (offline)" : "");
    ...
    static bool HandleNpcBotCommandReBindCommand(ChatHandler* handler, Optional<std::vector<std::string>> names)
    {
        ...
        static auto return_success = [](ChatHandler* chandler, Variant<std::string, uint32> name_or_count) -> bool {
            if (name_or_count.holds_alternative<uint32>())
                ...
                chandler->PSendSysMessage("Successfully re-bound {}", name_or_count.get<std::string>().c_str());
            return true;
        };
        ...
        if (!names || names->empty())
        {
                ...
                BotDataMgr::SelectNpcBotData(bot->GetEntry())->owner == owner->GetGUID().GetCounter())
            {
                ...
                {
                    handler->PSendSysMessage("Failed to re-bind {} for some reason!", bot->GetName().c_str());
                    handler->SetSentErrorMessage(true);
                    return false;
                }
                return return_success(handler, { bot->GetName() });
        ...
        for (decltype(names)::value_type::value_type name : *names)
        {
                ...
                BotDataMgr::SelectNpcBotData(bot->GetEntry())->owner == owner->GetGUID().GetCounter())
            {
                if (mgr->RebindBot(const_cast<Creature*>(bot)) != BOT_ADD_SUCCESS)
                {
                    handler->PSendSysMessage("Failed to re-bind {} for some reason!", name.c_str());
                    handler->SetSentErrorMessage(true);
                    continue;
                }
                ++count;
    ...
    static bool HandleNpcBotCommandUnBindCommand(ChatHandler* handler, Optional<std::vector<std::string>> names)
    {
        ...
        static auto return_success = [](ChatHandler* chandler, Variant<std::string, uint32> name_or_count) -> bool {
            if (name_or_count.holds_alternative<uint32>())
                ...
                chandler->PSendSysMessage("Successfully unbound {}", name_or_count.get<std::string>().c_str());
            return true;
        };
    ...
    static bool HandleNpcBotReviveCommand(ChatHandler* handler)
    {
        ...
        if (Player* master = u->ToPlayer())
        {
            if (!master->HaveBot())
            {
                handler->PSendSysMessage("{} has no npcbots!", master->GetName().c_str());
                handler->SetSentErrorMessage(true);
                return false;
            }
        ...
        else if (Creature* bot = u->ToCreature())
        {
            if (bot->GetBotAI())
            {
                if (bot->IsAlive())
                {
                    handler->PSendSysMessage("{} is not dead", bot->GetName().c_str());
                    handler->SetSentErrorMessage(true);
                    return false;
                }

                BotMgr::ReviveBot(bot, (bot->GetBotOwner() == owner) ? owner->ToUnit() : bot->ToUnit());
                handler->PSendSysMessage("{} revived", bot->GetName().c_str());
                return true;
            }
        }
    ...
    static bool HandleNpcBotAddCommand(ChatHandler* handler)
    {
        ...
        {
            handler->PSendSysMessage("{} is now your npcbot", bot->GetName().c_str());
            return true;
        }
